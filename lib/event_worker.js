...
class EventWorker {
  ...
  async check_room(event) {
    ...
    if (event.type === 'room_created' && event.room_name) {
      const userInfo = event.user_id ? UserInfo({ user_id: event.user_id, session_id: event.actor_id }) : {};
  ...
  async check_session(event) {
    ...
    if (event.type === 'user_offline') {
      ...
      if (Config.billing.activity) {
        await this.redis_rpush('customer_activity', JSON.stringify({
          type: 'session',
          org_id: event.org_id,
          sdk_version: event.sdk_version,
          created_at: event.created_at
