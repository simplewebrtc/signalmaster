...
class Occupants {
  ...
  async results(db) {
    ...
    for (const [occupant, count] of this.occupantCounts) {
      ...
      const data = await db.sessions.findOne({ id: occupant });
    ...
    for (const occupant of occupants) {
      const ua = (occupant.data || {}).browser;
      if (ua) {
        const info = JSON.parse(ua);

        occupant.browser = info.name;
        occupant.browserVersion = info.major;
      }
    }
