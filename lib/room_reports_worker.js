...
class RoomReporter {
  ...
  constructor(options) {
    ...
    this.interval = 1 * 60 * 1000; // wait 1 minute to process a given room
  ...
  async process_room(room_event) {
    ...
    const reports = {
      ...
      duration: (room.ended_at - room.created_at) / 1000  // Keep duration in seconds
    };
    for (const [reportType, reporter] of reporters) {
      reports[reportType] = await reporter.results(this.db);
    }

    let success = true;
    try {
      await this.db.rooms.update({ id: room.id }, {
        reports,
        reported_at: reports.reported_at
      });
      if (Config.billing.roomReports) {
        await this.redis_rpush('billing_room_reports', JSON.stringify(reports));
      }
    }
    catch (e) {
      console.log(e.stack);
