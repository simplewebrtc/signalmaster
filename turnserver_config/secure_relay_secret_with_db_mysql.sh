...
PATH="./bin/:../bin/:../../bin/:${PATH}" turnserver -v --syslog -a --max-bps=3000000 -f -m 3 --min-port=32355 --max-port=65535 --use-auth-secret --realm=realm --mysql-userdb="host=localhost dbname=coturn user=<user> password=<user> connect_timeout=30" --cert=<ssl-bundle.crt> --pkey=<privatekey.pem> --log-file=stdout --cipher-list=ALL $@
